...
<script>
  import {getAuthServerHardStatusCpuStatusPageList} from '@/api/ServerInfomation'
  export default {
    name: "cpu",
    components: {
    },
    ...
    methods:{
      ...
      //处理页面初始数据
      dealData(){
        getAuthServerHardStatusCpuStatusPageList('','',this.currentPage,this.size).then(request=>{
      ...
      Find(){
        if(this.serverIp||this.serverName !== '') {
          getAuthServerHardStatusCpuStatusPageList(this.serverIp,this.serverName,this.currentPage,this.size).then(request=>{
            this.totalNumber = request.data.body.total;
            this.tableData = request.data.body.data;
            for(let i=0;i<request.data.body.data.length;i++){
              this.tableData[i].ratio = Math.round(this.tableData[i].ratio*100)/100;
